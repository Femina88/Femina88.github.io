...
function ploitTitle (title) {//当切换歌曲时，利用歌曲列表title来获取
  ...
  musicAuthor.innerText = title.split("-")[0];
   musicName.innerText = title.split("-")[1].split(".")[0];
}
...
function setBgImage () {//背景图片的命名格式必须为:bgX.jpg(如：bg0.jpg)
  index=index+1;
  var url = audio.src;
  var bfURl = audio.src.substring(0,audio.src.lastIndexOf('/')+1);
  var bgName="bg"+((index)%7); 
  contanier.style.background='url('+bfURl+bgName+'.jpg)';
